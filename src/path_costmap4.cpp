...
* @author Shunya Hara
* @date 2022.4.8
* @details waypoint/path上が最小になるようなコストマップ KDTreeを用いて高速化したバージョン
*/
...
int main(int argc, char **argv){
    ...
    while (n.ok())  {
        ...
        //waypoint --> pcl
        pcl::PointCloud<pcl::PointXY>::Ptr waypc (new pcl::PointCloud<pcl::PointXY>); // waypoint cloud
        waypc->points.resize(uplimit-downlimit);
        for(int i=downlimit;i<uplimit;i++){
            waypc->points[i-downlimit].x = path.poses[i].pose.position.x;
            waypc->points[i-downlimit].y = path.poses[i].pose.position.y;
        }
        
        //KDTree
        pcl::KdTreeFLANN<pcl::PointXY>::Ptr waytree (new pcl::KdTreeFLANN<pcl::PointXY>);
        waytree->setInputCloud(waypc);

        double radius = sqrt((width*width+height*height)*0.25); //探索半径r
        ...
        if(path.poses.size()>2){
            ...
            for(int gy=0;gy<gw;gy++){
                for(int gx=0;gx<gh;gx++){
                    ...
                    //中心座標
                    pcl::PointXY grid_pos;
                    grid_pos.x=robotpose.position.x+double(gx)*resolution+map_x0;
                    grid_pos.y=robotpose.position.y+double(gy)*resolution+map_y0;

                    //最近傍点探索
                    waytree->radiusSearch(grid_pos, radius, k_indices, k_sqr_distances, max_nn);
                    if(k_indices.size() == 0){
                        break;
                    }
                    std::cout<<k_indices[0]<<std::endl;
                    //double nn_online_dis_min=k_sqr_distances[0];
                    geometry_msgs::Pose nn_online_pose;
                    double nn_online_dis_min=line_point_nn_pose(path.poses[k_indices[0]+downlimit].pose,path.poses[k_indices[0]+1+downlimit].pose,costmap_pose,nn_online_pose);
                    /*
